...
class CsvImportConnector(BaseConnector):
    ...
    def _get_file_info_from_vault(self, action_result, vault_id, file_type=None):
        ...
        try:
            ...
            return RetVal(action_result.set_status(phantom.APP_ERROR, f"Error retrieving file from vault: {e!s}"), None)
    ...
    def _handle_create_csv(self, param):
        ...
        ret_val, response = self._make_rest_call(f"/rest/artifact?_filter_container_id={container_id}&page_size={page_size}", action_result)
        if phantom.is_fail(ret_val):
        ...
        with open(file_loc, "w") as csvfile:
            ...
            for row in response["data"]:
                writer.writerow(row)
        vault_ret_dict = phantomrules.vault_add(container=container_id, file_location=file_loc, file_name=filename)
        if vault_ret_dict[0]:
        ...
        return action_result.set_status(phantom.APP_ERROR, f"Error adding file to vault: {vault_ret_dict}")
    ...
    def _handle_ingest_csv(self, param):
        ...
        if phantom.is_fail(ret_val):
            return action_result.set_status(phantom.APP_ERROR, f"Unable to find container: {message}")
        ...
        if vault_id:
            ...
            self.save_progress(f"****File Info : {file_info}")
            csv_vault_path = file_info["path"]
            with open(csv_vault_path) as csvf:
                artifacts_list = []
                try:
                    ...
                    if artifacts_list:
                        ...
                        if phantom.is_fail(create_artifacts_status):
                            self.debug_print(f"Error saving artifacts: {create_artifacts_msg}")
                            return action_result.set_status(phantom.APP_ERROR, f"Error saving artifacts: {create_artifacts_msg}")

                except Exception:
                    return action_result.set_status(phantom.APP_ERROR, f"Error while performing file operation. File:{csv_vault_path}")
